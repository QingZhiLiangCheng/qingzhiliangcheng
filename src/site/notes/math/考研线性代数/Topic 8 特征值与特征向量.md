---
{"created":"2025-12-01T11:13","updated":"2026-05-28T17:16","dg-publish":true,"permalink":"/math/考研线性代数/Topic 8 特征值与特征向量/","dgPassFrontmatter":true,"noteIcon":"","dg-note-properties":{"created":"2025-12-01T11:13","updated":"2026-05-28T17:16"}}
---
...
**结论：秩为1的行列式的特征值 快速写**
有一个结论：r(A)=1, 一个特征值是迹 剩下的都是0

### 特征值之间的关系
![Pasted image 20260528161911.png](/img/user/accessory/Pasted%20image%2020260528161911.png)

假设A的特征值为$\lambda$ 特征向量为x
**kA的特征值是$k\lambda$ 特征向量为x**
$$
kAx=k(\lambda x)=(k\lambda) x
$$

**$A^k$的特征值是$\lambda^k$,特征向量为x**
$$
A^kx=A^{k-1}(Ax)=A^{k-1}\lambda x=\lambda A^{k-1}x=...=\lambda^kx
$$

**A-kE的特征值是$\lambda -k$, 特征向量是x**
前面证明过了 这里再证一遍
$$
(A-kE)x=Ax-kx=\lambda x-kx=(\lambda-k)x
$$

**$A^{-1}$的特征值是$\frac{1}{\lambda}$ 特征向量为x**
![Pasted image 20260528163054.png\|300](/img/user/accessory/Pasted%20image%2020260528163054.png)
注意$\lambda$想要除过去 $\lambda \neq 0$ ! 所以说可逆矩阵的特征值不能为0！！

**$A^*$的特征值和特征向量**
![Pasted image 20260528163140.png\|300](/img/user/accessory/Pasted%20image%2020260528163140.png)
既然这样 前面讲过 行列式实际上就是特征值的乘积  所以伴随矩阵的特征值就是A矩阵的其他n-1个特征值的乘积！

相似矩阵特征值和特征向量见[[math/考研线性代数/Topic 10 相似#相似矩阵\|Topic 10 相似#相似矩阵]]