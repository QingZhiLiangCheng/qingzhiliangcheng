---
{"created":"2025-10-12T20:13","updated":"2026-03-20T08:48","dg-publish":true,"permalink":"/Computer Networking A Top-Down Approach/2.1 应用层协议原理/","dgPassFrontmatter":true,"noteIcon":"","dg-note-properties":{"created":"2025-10-12T20:13","updated":"2026-03-20T08:48"}}
---
...
问题2: 应用通过层间接口所提供的服务向对方发送报文，那下层所提供的形式是什么？下层所提供的服务在哪里能够调用？ --  通过层架接口上的SAP，通过SocketAPI这样的形式(一种原语)来访问传输层向上层所提供的服务
...
> [!quote]
> 原书说Socket就像一扇门或者管道，只管往Socket中推入
...
同时 在目标方 同源发往不同应用的实际上都是一条记录

#### 问题3：如何使用传输层提供的服务实现应用
##### 应用层协议
协议是对等层的实体在通信过程中应该遵守的规则集合 -- 规定了语法，语义，动作和次序
...
> [!quote]
> 前面提到了应用需求运输协议提供4个方面，分别是reliable data tansfer, throughput, timing and security. 前面提到了TCP的可靠性，通过增加TLS能够保证安全性，但是并没有保证throughput和timing

> [!quote]
> 原书中介绍了application-layer protocol -- 其实简单来说，前面学过什么是协议，协议是不同节点对等层实体所遵守的规则的集合，包括语法，语义和时序；应用层协议简单来说就是应用层实体所遵守的规则的集合，包括报文的语法，语义和时序...
> 有些应用层协议是由RFC文档定义的，是共有的 -- 比如现在用的HTTP(the HyperText Transfer Protocol)
> 有的应用层协议是专有技术，不公开
> 原书中强调了application和application-layer protocol的关系，applicaiton-layer protocol只是applicaiton的一部分，书中举了Web application的例子
> The Web applicaiton consists of many components, including a standard for document formats (that is, HTML), Web browers(for example, Chrome and Microsoft Internet Explorer), Web servers(for example, Apache and Microsoft servers), and an application-layer protocol.
